...
# XXD Panel 165｜中央の裂け目から現れる紙コラージュ
...
異なる元画像から個別に生成した Panel 165 の初回作例8点です。短い英文を使用。横長は1536×864の左右等分、縦長は1152×1536の上下等分です。

中央の縦長の裂け目、紙の毛羽立ち、重層コラージュを全点確認しました。一部は色が暗く、風景の細部を残しすぎ、車も写実的です。実際の結果の展示であり、全点が作風審査に合格したという意味ではありません。
...
中央の縦長の紙の裂け目が主役。内部を幾何学色面と紙の層で再構成し、繊維と柔らかな影を残します。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 165 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-165` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-165 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-165-prompt.en.md)／[中国語](references/xxd-panel-165-prompt.zh-CN.md)を参照してください。
